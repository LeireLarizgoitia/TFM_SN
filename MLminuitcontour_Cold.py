...
with open('COLD_ML_a.txt', "w") as file:
    for x in zip(*c):
        file.write("{0} {1}\n".format(*x))
...
#with open('COLD_Mass'+str(int(Mass_detector*1e-3))+'t_'+str(Field)+'Vcm_L'+str(ne_thres)+'electrons_res'+str(sigma0)+'_contour.txt', "w") as txt_file:
with open('COLD_contour_a.txt', "w") as txt_file:
    for line in grid1:
        content = str(line)
