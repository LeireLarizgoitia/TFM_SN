...
def flux_sum_prima(E,E_av,alpha):  #Fluxes following a continuous distribution. Normalized
    NN = (alpha + 1)**(alpha + 1) /(E_av * gamma(alpha + 1))
    phi =  NN * (E/E_av)**alpha * np.exp(-(alpha + 1)*(E/E_av)) #energy spectrum
    f = phi
    return f
...
def differential_events_flux_sum_prima(T,Eav,alpha,N,M,Rn2,Rn4):
    nsteps = 20
    int=  np.zeros((nsteps+1),float)
    EE = np.zeros((nsteps+1),float)
    "Integral Bounds"
    ...
    for i in range (0,nsteps+1):
        EE[i] = Emin + (Emax - Emin)/nsteps * i
        int[i] = (cross_section(T,EE[i],N,M,Rn2,Rn4) * flux_sum_prima(EE[i],Eav,alpha))
    return (np.trapz(int, x=EE))
...
def fnc_events_interval_obs_sum_prima(E,alpha):
    constant_usefull()
    binss,centres = binning()
    T_bins = []
    T_bins_plot = []
    ...
    for i in range(0,nsteps+1):
        t = T_thres + (T_max - T_thres)/nsteps * i
        T_true.append(t)
        dNdT.append( Mass_detector *1e3* Na /M_u * (differential_events_flux_sum_prima(t,E,alpha,N, M,Rn2,Rn4)))
        x = (1 - fnc_L(t))* t
        if x >= T_thres:
            T_true1.append(x)
            dNdT1.append( Mass_detector *1e3* Na /M_u * (differential_events_flux_sum_prima(t,E,alpha,N, M,Rn2,Rn4))) #Quenching applied
...
m = Minuit(fcn_np, (alpha_start, ev_start),name=("c","b")) #
